...
def get_urllist_one():
    links = []
    try:
        url = "http://www.55188.com/search.php?searchid=88&orderby=dateline&ascdesc=desc&searchsubmit=yes"
        html = myspyder(url)
        soup = bs(html.content, 'html5lib')
        ...
        # print(soup)
        for link in soup.find_all('a'):
            if re.match(u"viewthread.php\?tid=\d*&highlight=", link.get('href')):
                links.append('http://www.55188.com/'+link.get('href')[0:-11]+'&page=1&authorid=353347')
        links = list(set(links))
    except Exception as e:
        print(e)
    return links
