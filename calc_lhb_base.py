...
class lhb_calc:
    def __init__(self):
        ...
        self.alldata = pd.read_sql(self.sql,localconn()).values
        self.unit = pd.read_sql("select distinct `营业部代码`,`营业部名称` from `lhb`",localconn()).values
        self.list =[]
        self.days = 0
        self.all = pd.read_sql("SELECT * from `lhb` LEFT JOIN `lhbper` ON `lhb`.`code`=`lhbper`.`code` and `lhb`.`date`=`lhbper`.`date`",localconn())
