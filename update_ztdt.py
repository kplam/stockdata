...
"""
def zrzdt(stype,sdate,ser='both'):
    print(sdate,stype)
    ...
    try:
        ...
        if ser == 'local' or ser == 'both':
            try:
                df.to_sql('zrzdt', localconn(), flavor='mysql', schema='stockdata', if_exists='append', index=False)
            except Exception as e:
                print("local:%s[%s]"%(sdate,e))
        if ser == 'local' or ser == 'both':
            try:
                df.to_sql('zrzdt', serverconn(), flavor='mysql', schema='stockdata', if_exists='append', index=False)
            except Exception as e:
...
def zdtld(stype,sdate,ser='both'):
    ...
    try:
        ...
        js = json.loads(js)
        df = pd.DataFrame(js,columns=['code','name','close','percentage','fcb','flb','fdmoney','firsttime','lasttime','opentimes','amplitude','force'])
        df['firsttime'] = str(sdate) + ' ' + df['firsttime']
        df['lasttime'] = str(sdate) + ' ' + df['lasttime']
        if ser == 'local' or ser == 'both':
            try:
                df.to_sql('zdt', localconn(), flavor='mysql', schema='stockdata', if_exists='append', index=False)
            except Exception as e:
                print("local:%s[%s]"%(sdate,e))
        if ser == 'local' or ser == 'both':
            try:
                df.to_sql('zdt', serverconn(), flavor='mysql', schema='stockdata', if_exists='append', index=False)
            except Exception as e:
                print("server:%s[%s]"%(sdate,e))
        return df
    except Exception as e:
