...
sql_del = "delete from `dayline` WHERE `date`>='%s'"%(today)
# sql_dayline = "Select * from `dayline` WHERE `date`>='2017-11-10'"
sql_usefuldata = "Select * from `dayline` WHERE `date`>='%s'"%(today)
...
usefuldata.to_sql('dayline',local,flavor='mysql',schema='stockdata',if_exists='append',index=False,chunksize=1000)